...
namespace mlStringValidation
{
    ...
    public static class StringExtensions
    {
        ...
        public static IEnumerable<string> EndPointNames(this IEnumerable<PathBase> en)
        {
            var ret = new string[en.Count()];
            int x = -1;

            foreach (PathBase path in en)
            {
                ret[++x] = path.EndPointName;
            }
        ...
        public static byte[] GetFixedBytes(this string s) => s.GetFixedBytes(Encoding.UTF8.GetMaxByteCount(s.Length));
        public static int GetByteLength(this string s) => Encoding.UTF8.GetByteCount(s);
    }
}
